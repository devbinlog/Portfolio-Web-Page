...
선택 행동 자체가 개인화 데이터가 되어, 사용할수록 나를 닮아가는 AI 어시스턴트.`,
      description: `AI를 매일 썼지만, 대화가 쌓여도 달라지는 건 없었습니다.

"코드로 보여줘", "너무 길어", "단계별로 설명해줘" — 같은 요청을 매번 반복해야 했습니다. 개발 질문엔 코드 예제가 필요하고, 기획 논의엔 핵심 요점이 필요하다는 걸 AI는 기억하지 않았습니다.

동료한테는 이런 말 안 해도 됩니다. 함께 일하다 보면 상대방이 알아서 맞춰줍니다. AI는 왜 그러지 못하는 걸까.

정보를 저장하는 것과 행동이 변하는 것은 다릅니다. 내가 어떤 스타일의 답변을 선택했는지, 어떤 맥락에서 어떤 길이를 원했는지 — 이 패턴을 추론해서 다음 응답에 반영하는 AI는 없었습니다.

---

기존 해결책들의 한계는 같습니다 — 사용자가 직접 AI를 설정해야 한다는 것.

선호도를 텍스트로 적어두는 방식이 있습니다. 하지만 이건 내가 이미 내 선호도를 알고, 그걸 언어로 표현할 수 있고, 직접 설정하는 수고를 감수할 의향이 있을 때만 작동합니다.

실제로는 그렇지 않습니다. 사람들은 어떤 응답이 더 좋았는지 느끼고 선택합니다 — 말로 설명하지 않고.

또 다른 한계는 선호도가 맥락마다 다르다는 점입니다. 개발 질문과 기획 논의에서 원하는 스타일이 다릅니다. 정적인 설정으로는 이 차이를 담을 수 없습니다.

필요한 건 내가 설정하는 AI가 아니라, 내 행동을 관찰하고 스스로 학습하는 AI입니다.`,
      year: 2026,
